---
# title: Hypothetical interventions on workplace exposure with guaranteed positivity
fontsize: 11pt
header-includes:
- \allowdisplaybreaks
...
- \RedeclareSectionCommand[runin=false, beforeskip=-1sp,
...
format:
  ...
  #   reference-doc: "../../../../HeadRs/rmd_templates/style-guide-serif.docx"
tbl-cap-location: top
bibliography: "`r path.expand('~/Box Sync/papers/EnvEpi.bib')`"
notes-after-punctuation: true
csl: "`r path.expand('~/HeadRs/csl/de-gruyter.csl')`"
...
crossref:
  ...
  custom:
    ...
    - kind: float
      ...
      space-before-numbering: false
      latex-list-of-description: Supplemental Table
      caption-location: top
execute:
  freeze: auto
editor_options: 
...
library(xtable)
library(flextable)
set_flextable_defaults(font.size = 10, font.family = "Helvetica Neue")
library(data.table)
library(ggplot2); library(magrittr)
```

# Hypothetical interventions on workplace exposure with guaranteed positivity
...
![](resources/intervention-density-witnin-combos-a.pdf){#fig-1a}

![](resources/intervention-density-witnin-combos-b.pdf){#fig-1b}

![](resources/intervention-density-witnin-combos-c.pdf){#fig-1c}
...
tab1.sub |> 
  ...
  hline_bottom(part = "footer")
	...
	rownames(tab1.sub) <- NULL
	tab1.sub <- tab1.sub[,-4]
	tab1.sub[tab1.sub_hspace, "variable"] <- paste0(
		"\\hspace{2em}", tab1.sub[tab1.sub_hspace, "variable"])
	tab1.sub$spread <- sanitize(tab1.sub$spread)
	tab1.sub$spread.1 <- sanitize(tab1.sub$spread.1)
	names(tab1.sub) <- c(
		"%",
		"Count", "(\\%)",
		"Count", "(\\%)")
	print(xtable(tab1.sub,
							 align = "clrlrl"),
				add.to.row = list(
					pos = list(-1, 1, 16),
					command = c(
						paste0(
							"\\toprule & \\multicolumn{2}{c}{Study population}", 
							" & \\multicolumn{2}{c}{NHL Cases} \\\\ \n",
							"\\cmidrule(lr){2-3}\\cmidrule(lr){4-5}\n"),
						"\\midrule & Count & (\\%)     & Count & (\\%) \\\\\n\\midrule\n",
						"\\midrule & Median & (Q1, Q3) & Median & (Q1, Q3) \\\\\n\\midrule\n"
					)
				),
				hline.after = nrow(tab1.sub)
	)
}
...
:::{#fig-exposure}
![](../../resources/images/exposure.png){width="6.5in"}

Median annual average daily exposure to soluble, straight,
...
sol.tab |> flextable() |>
  ...
  hline_bottom(part = "footer")
	...
	names(sol.tab) <- gsub("CI.*)", "CI)", names(sol.tab))
	names(sol.tab) <- sanitize(names(sol.tab))
	names(sol.tab)[1] <- "Target exposure limit (mg/m\\textsuperscript{3})"
	xtable(sol.tab, align = c("clrlrlrl"))
}
```

Estimates of the counterfactual number of cases, number of cases 
averted, and cumulative incidence ratios (CIR) contrasting supportable 
